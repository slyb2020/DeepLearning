...
def draw_bbox(img, bbox):
    ...
    h, w = img.shape[0:2]
    n = bbox.size()[0]
    print("here bbox=", bbox.shape, h, w, n)
    print(bbox.detach().numpy())
    for i in range(n):
...
if __name__ == '__main__':
    ...
    model = torch.load(modelDir + "YOLOv1_epoch50.pkl")  # 加载训练好的模型
    for i, (inputs, labels) in enumerate(val_dataloader):
        print("labels = ", labels.shape)
        print("inputs = ", input, inputs.shape)
        inputs = inputs.cpu()
